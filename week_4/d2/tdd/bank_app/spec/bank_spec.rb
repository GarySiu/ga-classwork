...
let(:bank) { Bank.new('Bank of Jeremy') }
  ...
  it 'creates an account with a positive starting balance' do
    bank.create_account('Jeremy', 1)
    expect(bank.accounts['Jeremy']).to eq 1
  end

  it 'creates an account with a starting balance of 0' do
  ...
  it 'allows a customer to withdraw their moolah' do
